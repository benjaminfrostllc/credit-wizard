...
-- Update the trigger function (client_id set by separate trigger due to sequence permissions)
...
-- ============================================
-- 4b. SEPARATE TRIGGER TO SET CLIENT_ID
-- ============================================

-- This runs after profile insert to set client_id (workaround for sequence permissions)
CREATE OR REPLACE FUNCTION public.set_client_id()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.role = 'client' AND NEW.client_id IS NULL THEN
    UPDATE public.profiles
    SET client_id = 'CW-' || EXTRACT(YEAR FROM CURRENT_DATE)::TEXT || '-' || LPAD(nextval('public.client_id_seq')::TEXT, 3, '0')
    WHERE id = NEW.id;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS on_profile_created_set_client_id ON public.profiles;
CREATE TRIGGER on_profile_created_set_client_id
